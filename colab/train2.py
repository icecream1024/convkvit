...
# 配置参数
bs = 32  # batch size
imsize = 224  # 必须是224x224
patch = 7  # patch size必须是16
...
def validate(net, valloader, criterion):
    ...
    # data_dir = '/kaggle/input/medical/lower-gi-tract'
    ...
    #             xticklabels=dataset.disease_classes,
    #             yticklabels=dataset.disease_classes)
    ...
    # 保存混淆矩阵图片
    # cm_path = 'val_confusion_matrix.png'
    # plt.savefig(cm_path)
    # plt.close()
    #
    # # 记录到wandb
    # wandb.log({"Validation Confusion Matrix": wandb.Image(cm_path)})
...
def test(net, testloader, criterion):
    ...
    #
    # # 记录到wandb
    # wandb.log({"Test Confusion Matrix": wandb.Image(cm_path)})
...
def main():
    ...
    # net = models.vgg19(num_classes=23).to(device)
    # net = CaiT(image_size=imsize, patch_size=patch, num_classes=23, dim=384, depth=6, heads=8,
    #            mlp_dim=1024, dropout=0.1, emb_dropout=0.1, dim_head=64, cls_depth=384).to(device)
    net = mobilevit_s().to(device)
    # 在训练前打印 FLOPs 和参数量
